"""Reusable PEFT / LoRANetwork → ComfyUI convert path.

``scripts/convert_lora_comfyui.py`` is the CLI for this module. There is no
second converter.
...
writes ``<name>_comfyui.safetensors`` plus a sidecar JSON beside the original.
Originals are never touched. CPU only.

Music 3 concept sliders (LoRANetwork ``lora_down``/``lora_up`` under
``lora_unet-`` / ``lora_te-``) use the same path: detect ``music3`` /
``music3_lm`` and, for the DiT, fuse ``to_q``/``to_k``/``to_v`` into
ComfyUI's ``self_attn.to_qkv``. Anima/Krea/Music 3 block mappings that already
work are not rewritten.
...
# Official Hub PEFT dumps (e.g. Comfy-Org/Krea-2 style LoRAs) prefix modules
# with ``transformer.`` instead of ``base_model.model.``.
...
# Sidecar fields written next to every converted file. ``unit_scale`` is True
...
# Inverse of krea_weights._TOP linear entries. Official Comfy-Org turbo LoRAs
# include these roots; conceptmod's default target_modules do not train them.
KREA_ROOT = {
    "img_in": "first",
...
# file. anima is verified; krea block names match Comfy-Org/Krea-2
# ``loras/krea2_turbo_lora_rank_64_bf16.safetensors``. music3 is derived from
...
def map_krea(module: str):
    ...
    if module in KREA_ROOT:
        return KREA_ROOT[module]
...
# Qwen-Image / Qwen-Image-Edit share QwenImageTransformer2DModel. ComfyUI
# (comfy/ldm/qwen_image/model.py) keeps the same module names as diffusers.
QWEN_ROOT = {
    "img_in", "txt_in", "txt_norm", "proj_out",
}
QWEN_ROOT_PREFIXES = ("time_text_embed.", "norm_out.")


def map_qwen(module: str):
    """Map a PEFT Qwen-Image module onto its ComfyUI name (identity)."""
    if module.startswith("transformer_blocks."):
        return module
    if module in QWEN_ROOT:
        return module
    if module.startswith(QWEN_ROOT_PREFIXES):
        return module
    return None
...
def _strip_peft_default(module: str) -> str:
    """Drop PEFT adapter-name segments (``.default``) from a module path."""
    parts = [p for p in module.split(".") if p != "default"]
    return ".".join(parts)
...
def split_lora_key(key: str):
    ...
    elif key.startswith(TRANSFORMER_PREFIX):
        key = key[len(TRANSFORMER_PREFIX):]
    for tail, side in _LORA_SIDES:
        if key.endswith(tail):
            return _strip_peft_default(key[: -len(tail)]), side
    # PEFT sometimes writes ``lora_A.default.weight`` (adapter name in between).
    for side_name, side in (("lora_A", "lora_A"), ("lora_B", "lora_B"),
                            ("lora_down", "lora_A"), ("lora_up", "lora_B")):
        token = f".{side_name}.default.weight"
        if key.endswith(token):
            return _strip_peft_default(key[: -len(token)]), side
    return None
...
def detect_backend(cfg, keys):
    ...
    if any(re.match(r"transformer_blocks\.\d+\.attn\.to_gate", s) for s in stems):
        return "krea", "key heuristic (transformer_blocks.N.attn.to_gate)"
    if any(re.search(r"\.(add_q_proj|img_mlp|txt_mlp|img_mod)\b", s) for s in stems):
        return "qwen", "key heuristic (qwen dual-stream attn/mlp)"
    if any(re.match(r"transformer_blocks\.\d+\.attn\.", s) for s in stems):
        # Shared by krea (default conceptmod targets) and qwen. Prefer krea
        # when the distinctive Qwen/Krea extras are absent so existing
        # adapter_config-less krea dumps keep working; pass --backend qwen
        # when the file is a Qwen attn-only LoRA.
...
def sidecar_metadata(backend: str, cfg: dict, *, n_keys: int | None = None):
    """JSON payload written next to each converted safetensors."""
    rank = cfg.get("r")
    alpha = cfg.get("lora_alpha")
    if backend in FUSED_QKV:
        unit_scale = True
    elif rank is not None and alpha is not None:
        unit_scale = alpha == rank
    else:
        unit_scale = True
    payload = {
        "arch": backend,
        "fused_qkv": backend in FUSED_QKV,
        "host": HOST.get(backend, "dit"),
        "unit_scale": unit_scale,
        "recommended_range": list(RECOMMENDED_RANGE.get(backend, [0.5, 1.5])),
    }
    if n_keys is not None:
        payload["n_keys"] = int(n_keys)
    return payload


def write_sidecar(out_path: str, payload: dict) -> str:
    side_path = out_path + SIDECAR_SUFFIX
...
def pattern(key: str) -> str:
    ...
    LoRA side names are normalized so PEFT ``lora_A``/``lora_B`` compare equal
    to Comfy-Org ``lora_down``/``lora_up`` (same module, different convention).
    """
...
def main():
    ...
    for path in candidates:
        ...
        side = write_sidecar(
            out_path, sidecar_metadata(backend, cfg, n_keys=len(tensors)))
